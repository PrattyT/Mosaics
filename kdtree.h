...
public:
   ...
   * true cout << shouldReplace(target, currentBest2, possibleBest2) << endl; //
   * false cout << shouldReplace(target, currentBest3, possibleBest3) << endl;
   ...
   * (https://courses.engr.illinois.edu/cs225/sp2019/mps/mosaics/) in the MP
   * instructions.
   *
   * @todo This function is required for Part 1.
...
private:
  ...
  // helper for distance
  double distanceSq(const Point<Dim> &left, const Point<Dim> &right) const;
  ...
  int select(int left, int right, int k, int dim);
  int partition(int l, int r, int pivot, int dim);
  void swap(int l, int r);

  Point<Dim> findNearestNeighbor(const Point<Dim> &query, int d,

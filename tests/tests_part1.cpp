...
void compareBinaryFiles(string yourFile, string ourFile) {
  ...
  if (!tree_equals_output(ours, yourFile))
    FAIL("Your output in \"" + yourFile + "\" does not match our output in \"" +
         ourFile + "\"");
}

TEST_CASE("KDTree::testSmallerDimVal Tests", "[custom][weight=1][part=1]") {
  vector<Point<3>> pts;
  KDTree<3> tree(pts);
  Point<3> a(1, 2, 3);
  Point<3> b(3, 2, 1);
...
TEST_CASE("KDTree::shouldReplace Tests", "[custom][weight=1][part=1]") {
  vector<Point<3>> pts;
  KDTree<3> tree(pts);
...
TEST_CASE("KDTree constructor, 1D (Dim=1)", "[custom][weight=1][part=1]") {
  _test_linear_constructor<1>(10);
}

TEST_CASE("KDTree constructor, 3D (Dim = 3)", "[custom][weight=1][part=1]") {
...
TEST_CASE("KDTree::findNearestNeighbor (2D), returns correct result",
  ...
  int K = 2;
  std::string fname =
      "test_result_kdtree_" + to_string(K) + "_" + to_string(size) + ".kd";
  writeKdTreeToFile(tree, fname);
  compareBinaryFiles(fname, "tests/expected_kdtree_" + to_string(K) + "_" +
                                to_string(size) + ".kd");
 ...
 */
template <int Dim> class MineActionFAIL : public Point<Dim>::MineAction {
public:
